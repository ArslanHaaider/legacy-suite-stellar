...
pub fn add_asset(
    ...
    let beneficiary_struct = BenificaryStorage {
        token: token_address.clone(),
        benificary: benificary.clone(),
        value: amount,
        claimed: false,
    };
    benificary_assets.append(&vec![&env, beneficiary_struct]);

    let mut asset_array: Vec<(Address, Address, i128, bool)> = asset_map
        .get(benificary.clone())
        .unwrap_or(default_asset_array);
    asset_array.append(&vec![
        &env,
        (token_address.clone(), from.clone(), amount, claimed),
    ]);
    ...
    pub fn claim_asset(
        ...
        if env.storage().persistent().has(&from) {
            ...
            for asset in asset_array.clone() {
                ...
                let index = asset_array.first_index_of((
                    token_address.clone(),
                    from.clone(),
                    amount,
                    claimed,
